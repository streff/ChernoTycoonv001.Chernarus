...
_fuelPrice = _this select 3;
_caller = _this select 4;
_servicegroup = group _serviceman;
_servicepad = _serviceman getVariable "OSMO_SV_servicepad";
_employer = name leader _caller;
_payCheck = call compile format["TOT_%1_payOsmo", _employer];
...
if((!isEngineOn _vehicle) && ((_vehicle distance _servicepad) < OSMO_SV_radius)) then
{
 ...
 private["_hitpoints", "_heliName", "_servicetime", "_serviceReport"];
  ...
  //--- Report header
	...
	//TOT COST ADDED ------------------------------------------+++--------------------------
	_totrepaircost = (_servicetime * 10) + 100;
	_allincost = floor(_totrepaircost + _fuelPrice);
  ...
  _svTime = _servicetime; // keep seconds
  ...
  {
    _inspectText = _inspectText + format["Here is the report. Total: %1", _allincost, " Would you like to continue?."];
  };
...
//_TOTresponseText = format["Thanks, received $%1.", _allincost];
//"TOT_PayOsmo" addPublicVariableEventHandler {[[_serviceman, _TOTresponseText], "OSMO_SV_serviceChat"] call BIS_fnc_mp; };
[[_caller, "scripts\tot\payOsmo.sqf", "Pay for Fuel and Repairs", [_caller, _serviceman, _allincost]], "TOT_fnc_MPAddactionParam", nil, true, true] spawn BIS_fnc_MP;
//_caller addAction["Pay for Fuel and Repairs", "scripts\tot\payOsmo.sqf", [_serviceman, _allincost]];

[[_serviceman, "Scroll your action menu to pay, or wait 10 sec to cancel."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;

_timer = time + 10;

waituntil{(time > _timer) || (call compile format["TOT_%1_payOsmo", _employer] > 0)};

[[_caller], "TOT_fnc_MPRemoveaction",false, true] call BIS_fnc_MP;

}; //end service inspection and report section
//------------------------------------------------------------------------

if (call compile format["TOT_%1_payOsmo", _employer] > 0) then {
[[_serviceman, "Starting Repairs."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;

//reset tot pay toggle
call compile format["TOT_%1_payOsmo = 0;", _employer];
call compile format["publicVariable 'TOT_%1_payOsmo'", _employer];
//[[_serviceman, "Toggle reset."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;

  hint format["Servicetime: %1", _svTime];
 // Sleep for the service time - _svTime is in seconds

	if (_svTime < 300) then {
	_svTimeOut = time + _svTime;
	_svHalfTime = time + (_svTime / 2);
	waitUntil {sleep 5; time > _svHalfTime};
	[[_serviceman, "Half way there..."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;
	waitUntil {sleep 5; time > _svTime};
	} else {
  	_svTimeOut = time + 300;
	_svHalfTime = time + 150; 
  waitUntil {sleep 5; time > _svHalfTime};
  [[_serviceman, "Half way there..."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;
  waitUntil {sleep 5; time > _svTimeOut};
  ...
  if(_svTime != 0) then
  {
	...
	[[_serviceman, "Ok. We'll pack up."], "OSMO_SV_serviceChat"] call BIS_fnc_mp;
	};
	sleep 1;
};  

//if player paid do the bit above here if not go straight to below
[[_serviceman, "end service"], "OSMO_SV_serviceChat"] call BIS_fnc_mp;
